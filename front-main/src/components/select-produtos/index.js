...
const SelectProdutos = (props) => {
    const fetchData = async (inputValue) => {
        configAxios.params = { term: inputValue };
        return (await axios.get(`${BASE_URL}/data/produto-select`, configAxios))
            .data;
    };
    ...
    const formatOptionLabel = ({ produto, descricao }) => (
        <div>
            <div className="clearfix" />
            <div>
                {produto} <span className="text-muted">{descricao}</span>{' '}
            </div>
        </div>
    );
