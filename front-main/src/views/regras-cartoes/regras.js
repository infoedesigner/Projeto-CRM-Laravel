...
import RegrasListSaveCartoes from "../../components/regras-list-save-cartoes";

const RegrasNegocioCartoes = (props) => {
    ...
    const GET_DATA_REGRAS = async () => {
        await axios.get(`${BASE_URL}/data/v1/regrasTabela-cartao/${tabelaId}`)
            .then((response) => {
    ...
    const handleSubmit = async (values) => {
        setLoading(true);
        await axios
            .post(`${BASE_URL}/data/v1/add-regrasTabela-cartao`, values, configAxios)
            .then((response) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: yup.object({
            ...
            fator: yup.string().required('O campo é obrigatório.'),
        }),
        onSubmit: (values) => {
            handleSubmit(values);
        },
    ...
    const delRegra = async (id_regra_produto) => {
        ...
        await axios
            .get(`${BASE_URL}/data/v1/delRegrasProduto-cartao/${id_regra_produto}`)
            .then(() => {
                GET_DATA_REGRAS();
                setLoading(false);
            })
        ...
        <>
                ...
                <Col sm={12} md={12} lg={12} xl={2}>
                    <Form.Group>
                        ...
                        <Form.Control
                            ...
                            value={formik.values.fator}
                            className={`form-control ${
                                formik.touched.fator &&
                                formik.errors.fator
                                    ? 'is-invalid'
                                    : ''
                            }`}
            ...
            {dataRegras?.length > 0 ? (
                dataRegras.map((i, key) => {
                    return <RegrasListSaveCartoes key={key} item={i} delRegra={delRegra} />;
                })
            ) : (
                <>Nenhuma regra</>
            )}
...
export default RegrasNegocioCartoes;
