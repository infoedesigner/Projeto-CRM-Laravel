...
const Regras = (props) => {
    ...
    const [valorRegra, setValorRegra] = useState(0);

    const GET_DATA_REGRAS = async () => {
        await axios.get(`${BASE_URL}/data/v1/regrasTabela/${tabelaId}`)
            .then((response) => {
    ...
    const handleSubmit = async (values) => {
        setLoading(true);
        await axios
            .post(`${BASE_URL}/data/v1/add-regras-tabela`, values, configAxios)
            .then((response) => {
    ...
    const delRegra = async (id_regra_produto) => {
        ...
        await axios
            .get(`${BASE_URL}/data/v1/delRegrasProduto/${id_regra_produto}`)
            .then(() => {
                GET_DATA_REGRAS();
                setLoading(false);
            })
        ...
        <>
            ...
            {dataRegras?.length > 0 ? (
                dataRegras.map((i, key) => {
                    return <RegrasListSave key={key} item={i} delRegra={delRegra} />;
                })
            ) : (
                <>Nenhuma regra</>
            )}            
