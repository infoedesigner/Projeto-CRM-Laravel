...
export function EditRegrasCartoes(props) {
    const [loading, setLoading] = useState(false);

    const { GET_DATA, id } = props;
    const handleSubmit = async (values) => {
        setLoading(true);
        await axios
            .patch(`${BASE_URL}/data/v1/regras-cartoes/${id}`, values, configAxios)
            .then((response) => {
    ...
    useEffect(async()=>{
        ...
        await axios
            .get(`${BASE_URL}/data/v1/regras-cartoes/${id}`, configAxios)
            .then((response) => {
                formik.setValues({
                    id_produto: response.data.id_produto,
                    produto: {value: response.data.id_produto, label: response.data.produto},
                    nome_regra: response.data.nome_regra,
                    tipo: response.data.tipo,
                    tipo_label: {value: response.data.tipo, label: response.data.tipo},
                });
            })
            .then(() => {
                setLoading(false);
                    ...
                    <Row className="mb-3">
                        ...
                        <Col sm={6} md={6} lg={6}>
                            <Form.Group>
                                ...
                                }} valor={formik.values.tipo_label}/>
                            </Form.Group>
                        </Col>
                    </Row>
